...
test_suit_book_roundtrip :: proc(t: ^testing.T) {
	...
	testing.expectf(t, miss == 0, "%d/%d entries failed round-trip lookup", miss, len(book_entries))
}
...
test_suit_book_known :: proc(t: ^testing.T) {
	...
	a, oka := lookup(0x1A00, 0x0003)
	b, okb := lookup(0x1A00, 0x0028)
