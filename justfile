...
#
# The APPS that read this corpus -- the panel quiz, the optimal-point-count app and the six
# datastar quiz ports -- live in their own repo now: ~/dev/bridge-system-apps (`just quiz`,
# `just dsquiz serve`, ... are run from there). They find this corpus through BRIDGE_SYSTEM_HOME,
# which defaults to this checkout's path; nothing here points back at them. `just mod` paths are
# literal strings -- no variable, no env-var expansion -- so a cross-repo module would have had to
# hard-code a machine-specific path, which is why the split is clean rather than cross-linked.
#
...
# `uv run --script`, not a bare `uv run`: the root pyproject.toml left with the apps, and both of
# these scripts carry their own PEP 723 inline metadata (docopt), so they resolve standalone.
...
_py-regen:
    ...
    uv run --script regen-html-deals.py {{deals_output_dir}}
...
_py-run-scratch TCL_SCRIPT:
    cd {{justfile_directory()}}/deal-simulations/tcl-sims
    uv run --script run-deal.py --deal-count 48 --deal-script-path {{justfile_directory()}}/deal-simulations/tcl-sims/{{TCL_SCRIPT}} --html-output-path {{justfile_directory()}}/{{TCL_SCRIPT}}.html
